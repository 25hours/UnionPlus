...
class ExpressionProcess(object):
    ...
    def load_data_from_redis(self):
        ...
        try:
            ...
            print("not enough data for this time range")
    ...
    def process(self):
        ...
        if single_expression_calc_res: #确保上面的条件 有正确的返回
            res_dic = {
                ...
                'calc_res_val':single_expression_calc_res[1],   #服务触发报警时的数据最大值
    ...
    def get_avg(self,data_set):
        ...
        :return:
        '''
        clean_data_list = []    #用于存储cpu，mem等没有子项目的数据
        clean_data_dic = {}     #用于存储网卡等，有子项目的数据
    ...
    def judge(self,calculated_val):
        ...
        calc_func = getattr(operator,self.expression_obj.operator_type)     #operator用于反射计算符号
