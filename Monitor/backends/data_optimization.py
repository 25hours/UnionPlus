...
class DataStore(object):
    ...
    def __init__(self, client_id,service_name, data,redis_obj):
        ...
        self.process_and_save()     #在实例化时就调用对应函数进行数据优化存储
    ...
    def process_and_save(self):
        ...
        if self.data['status'] ==0:# service data is valid判断数据合法性
            for key,data_series_val in settings.STATUS_DATA_OPTIMIZATION.items():
                data_series_key_in_redis = "StatusData_%s_%s_%s" %(self.client_id,self.service_name,key)    #数据存入redis中
                ...
                if not last_point_from_redis: #this key is not exist in redis
                    ...
                    #the data got saved last time，刚启动时没有值因此初始化值，初始化的值要包含时间，后面存入数据时会根据时间戳以及设定间隔判断是否优化
                    self.redis_conn_obj.rpush(data_series_key_in_redis,json.dumps([None,time.time()] ))     #此处相当于append添加数据
                if data_series_val[0] == 0:#this dataset is for unoptimized data, only the latest data no need optimiaztion，若为最新数据，则不需要优化
    ...
    def get_optimized_data(self,data_set_key, raw_service_data):
        ...
        service_data_keys = raw_service_data[0][0].keys() #[iowait, idle,system...]，监控指标
        first_service_data_point = raw_service_data[0][0] # use this to build up a new empty dic
        #print("--->",service_data_keys)
        optimized_dic = {} #set a empty dic, will save optimized data later，优化后的数据
