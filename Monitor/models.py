...
class Host(models.Model):
    ...
    ip_addr =  models.GenericIPAddressField(unique=True)    # ip地址
...
class ServiceIndex(models.Model):
    name = models.CharField(max_length=64)  # CPU的idle，iowait等可以自己定义名字
    key =models.CharField(max_length=64)    # 从客户端（监控项插件）返回的CPU的idle，iowait等监控指标的名字作为key值
    data_type_choices = (
        ('int',"int"),
        ('float',"float"),
        ('str',"string")
...
class Service(models.Model):    # cpu，内存，io等
...
class TriggerExpression(models.Model):  # 触发器表达式，反向关联Trigger
    ...
    logic_type = models.CharField(u"与一个条件的逻辑关系",choices=logic_type_choices,max_length=32,blank=True,null=True)  # cpu的idle<10 or iowait>20
    def __str__(self):
...
class Action(models.Model): # 定义报警，如CPU报警等
    name =  models.CharField(max_length=64,unique=True)
    ...
    operations = models.ManyToManyField('ActionOperation')  # 报警执行动作
...
class ActionOperation(models.Model):    # 告警升级，通过触发次数判断（step）
    name =  models.CharField(max_length=64)
    step = models.SmallIntegerField(u"第n次告警",default=1,help_text="当trigger触发次数小于这个值时就执行这条记录里报警方式")
    '''
    5 --> email,10 --> sms,15 --> weichat；前五次触发trigger则用email报警，5次到10次之间则触发sms报警
    '''
    action_type_choices = (
        ('email','Email'),
        ('sms','SMS'),
