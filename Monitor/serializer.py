...
class ClientHandler(object):
    def __init__(self, client_id):
        ...
        self.client_configs = {
            "services":{}   #定义待监控服务的字典方便后期取值
        }

    def fetch_configs(self):
        try:
            host_obj = models.Host.objects.get(id=self.client_id)
            template_list= list(host_obj.templates.select_related())    #取出主机包含的所有模板，并转化成列表形式

            for host_group in host_obj.host_groups.select_related():    #取出主机组中的模板，并追加到模板列表
